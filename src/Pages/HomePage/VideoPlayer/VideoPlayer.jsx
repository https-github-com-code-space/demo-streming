  ...
  return (
    <div className='w-[95%] md:w-[90%] mx-auto mt-28'>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 ">
        {videoData.map((video) => (
          <div key={video.id} className=" mx-auto shadow-lg shadow-[#4b69bd] rounded-lg px-2 py-2" onClick={() => openModal(video)}>
            <img className='cursor-pointer w-full' src={video.img} />
            <h2 className='text-xl text-[#4b69bd] font-bold mt-1 px-2'>{video.title}</h2>
          </div>
        ))}
      </div>
      <Modal className='m-20 bg-white border-none outline-none' isOpen={modalIsOpen} onRequestClose={closeModal}>
        <button className='ml-auto block text-3xl mb-1' onClick={closeModal}><MdClose></MdClose></button>
